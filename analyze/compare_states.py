...
def calculate_spread(games_df_path, states_to_compare):
    games_df = pandas.read_pickle(games_df_path)
    ...
    for state in [states_to_compare]:
        ...
        for other_state in STATES:
            ...
            if not pair_df.empty:
                try:
                    pair_df['rating_a'] = pandas.to_numeric(
                        pair_df['rating_a'])
                    pair_df['rating_b'] = pandas.to_numeric(
                        pair_df['rating_b'])
                ...
                expected_score = 1 / \
                    (1 +
                     (10 ** ((pair_values[:, 1] - pair_values[:, 0]) / 400.0)))
        ...
        try:
            mean_points = total_points_gained/total_samples
        except ZeroDivisionError:
            continue
        state_only.append([mean_points, total_samples])
